...
- 固定班会、阅读连堂、趣味课堂，并避开英语、语文、数学教研时间；已配置教师时，阅读自动随本班语文教师、班队会自动随本班班主任，未配置时教师留空但不阻止排课。
...
- 在同一“教师与任课”页面配置教师、班主任及班级任课关系；同一班同一课程可由多位教师按节数共同承担。
- 教师名单支持按模板批量导入 `.xlsx`，可同时填写姓名、期望最低周课时、班主任班级以及多个“班级：科目(2节)”任课关系；不写节数表示承担该科全部周课时，班级支持完整名称、`1.1` 数字简写和“一年级1-7班”范围。同一文件中的同名行自动合并为一位教师，课时数相加、任课配置合并；真实重名教师须在姓名中主动加注学科或编号区分。
- 批量导入时，同一课程分配合计超过课程标准会整份拒绝且不写入；部分分配不足时先显示缺口并要求明确二次确认，完全未配置教师的课程仍允许排课。
- 网页任课配置按“年级 → 班级”筛选，可通过姓名搜索教师、增减共同任课教师并填写各自节数；跨班级修改会暂存，最后一次性保存所有已修改班级。
- 班级课表按“年级 → 班级”两级选择查看，也可切换查看教师课表。
- 支持正式 A4 竖向打印班级课表和教师课表；班级课表显示科目及已分配教师，教师课表显示班级与科目。
- 班级课表可按全部年级或指定年级导出多页 PDF，也可导出每班一张工作表的 XLSX。
- 教师课表可按全部、年级或指定教师导出；PDF 批量导出为 ZIP，XLSX 批量导出为每位教师一张工作表。
- 所有 PDF 均为正式 A4 竖向、纯白底、左上角校徽及加粗居中标题的简洁样式。
- 生成后的班级课表支持连续交换多节课程、撤销与一次保存；保存前会检查教师时间、固定课位和教研避让等硬性冲突，教师课表同步更新。
...
“排课规则”页面可以下载不含已生成课表的配置 JSON；“生成课表”页面可以导出班级或教师 PDF/XLSX。
