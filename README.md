...
- 在同一“教师与任课”页面配置教师、班主任及班级任课关系。
- 教师名单支持按模板批量导入 `.xlsx`，可同时填写姓名、期望最低周课时、班主任班级以及多个“班级：科目”任课关系；班级支持完整名称、`1.1` 数字简写和“一年级1-7班”范围，教师学科从实际任课配置自动推导。
- 网页任课配置按“年级 → 班级”筛选，并可通过姓名搜索教师；跨班级修改会暂存，最后一次性保存所有已修改班级。
